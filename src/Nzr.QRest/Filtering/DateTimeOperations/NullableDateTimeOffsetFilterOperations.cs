...
/// </summary>
public record NullableDateTimeOffsetFilterOperations : BaseDateTimeFilterOperations<DateTimeOffset>, IFilterOperations<DateTimeOffset?>
{
    /// <summary>
    ...
    public Expression<Func<DateTimeOffset?, bool>>? ToExpression()
    {
        ...
        if (!IsNull.HasValue || !IsNull.Value || !IsNotNull.HasValue || IsNotNull.Value)
        {
            ...
            {
                expression = expression.And(value => value != null && DateOnly.FromDateTime(value.Value.DateTime) == DateEquals.Value);
            }

            if (TimeEquals.HasValue)
            {
                expression = expression.And(value => value != null && TimeOnly.FromDateTime(value.Value.DateTime) == TimeEquals.Value);
            }
    ...
    private Expression<Func<DateTimeOffset?, bool>> ApplyDateTimeNullCheck(Expression<Func<DateTimeOffset?, bool>> expression)
    {
        if (IsNull == true)
        {
